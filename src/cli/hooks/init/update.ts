	...
	if (!state.ready) {
		this.warn(
			`${state.name} update available from ${Chalk.greenBright(state.currentVersion)} to ${Chalk.greenBright(
				state.updateVersion,
			)}. Review the latest release and run "crypto-json-rpc update" once you wish to update.`,
		);

		cli.url(
			`Click here to read the changelog for ${state.currentVersion}.`,
			"https://github.com/ARKEcosystem/crypto-json-rpc/blob/master/CHANGELOG.md",
		);
	}
};
